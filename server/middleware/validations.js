...
export default class Validation {
    ...
    static addRecipeValidation(req, res, next) {
        ...
        if (title === undefined || ingredients === undefined || description === undefined) {
            res.status(400);
            res.json({
                status: 'Failed',
            ...
            if (!(validator.isEmpty(description))) {
                if (!(validator.isLength(description, { min: 20, max: 1000 }))) {
                    errors.description = 'Description provided must be more than 20 characters';
                }
            } else { errors.description = 'Description for your recipe are required'; }
