...
describe('test cases for voting a recipe', () => {
  ...
  describe('test cases for voting a recipe negative operations', () => {
    it('should not be able to vote a recipe when recipe id is not a' +
      'number', (done) => {
    ...
    it('should not be able to vote a recipe if recipe does not exit or' +
    ...
    it('should not be able to downvote the same recipe more than' +
      'once by user one', (done) => {
      request.post('/api/v1/recipes/4/vote?vote=downvote')
        .set('x-access-token', userOneToken.token)
        .end((error, response) => {
    ...
    it('should not be able to upvote the same recipe more than' +
      'once by user two', (done) => {
      request.post('/api/v1/recipes/4/vote?vote=upvote')
        .set('x-access-token', userTwoToken.token)
        .end((error, response) => {
          expect(response.status).to.equal(409);
